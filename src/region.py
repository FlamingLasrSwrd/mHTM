...
class Region(object):
    ...
    def save(self, name):
        """Save the HTM as a pickle.

        The save path will be in the log directory with the given name in a
        pickle file. Note that the ".pkl" extension will automatically be
        applied.
    ...
    def _log_stats(self, name, value):
        ...
        """
        with open(os.path.join(self.log_dir, 'stats.csv'), 'a') as f:
            writer = csv.writer(f)
            writer.writerow([name, value])


class SPRegion(Region):
    """Class for implementing a Spatial Pooler region.

    This implementation follows Numenta's original CLA whitepaper definition.
    It additionally meets the scikit-learn requirements. It also supports
    serialization.
    """
    ...
    def __init__(self,
                 ...
                 global_inhibition=True, trim=1e-4, disable_boost=False,
                 seed=None,
        ...
        **** The below parameters are for the region ****
        ...
        @param nactive: An alternative to pct_active, that uses a fixed
        number.
    ...
    def _init(self):
        ...
        This is pulled out to ensure that the class is compatible with
        scikit-learn.
        """
        ####
        # Parameter checks
        ...
        if self.random_permanence is None:
            ...
            for i, syn_d in enumerate(self.syn_dist):
                self.p[i] = pmin + pwin * (1 - (syn_d / float(max(self.ncolumns - i, i))))
    ...
    def get_params(self, deep=True):
        ...
        """
        return {'ninputs': self.ninputs,
                'ncolumns': self.ncolumns,
                'pct_active': self.pct_active,
                ...
                'nsynapses': self.nsynapses,
                'seg_th': self.seg_th,
                'syn_th': self.syn_th,
                'pinc': self.pinc,
                'pdec': self.pdec,
                'pwindow': self.pwindow,
                'random_permanence': self.random_permanence,
                'nepochs': self.nepochs,
                'clf': self.clf,
                'log_dir': self.log_dir}  # TODO use self.__dict__?
    ...
    def _compute_neighborhood(self):
        """Compute the neighborhood for each column as a bit mask.

        1D is being used, so neighbors are selected along a number line with
        the ends clamped.
    ...
    def _update_boost(self, min_dc):
        ...
        Boost linearly decreases with respect to the active duty cycle.
        If the active duty cycle is greater than the minimum duty cycle the
        boost is disabled (set to 1).
...
# TODO: Enforce order of function calls
    def _phase1(self):
        """Execute phase 1 of the SP region: compute the overlap.
    ...
    def _phase2(self):
        """Execute phase 2 of the SP region: compute the active columns.
        ...
        if self.global_inhibition:
            ...
            for i in range(self.ncolumns):
                ...
                if ix.shape[0] <= k:
                    ...
                    m = max(-np.partition(-self.overlap[ix, 0], k - 1)[k - 1], 1)

                # Set the column activity
                if self.overlap[i, 0] >= m:
                    self.y[i, 0] = True

    def _phase3(self):
        """Execute phase 3 of the SP region: learning.
    ...
    def score(self, x, y, tr_x=None, tr_y=None, score_method='column'):
        """Compute the accuracy for y.

        Note that if no classifier is specified, this function will return None.
        This method must be called after fitting the SP.
        ...
        if tr_y is None:
            tr_y = self.tr_y
        if (self.clf is None) or \
           (self.column_activations is None) or \
           (tr_y is None):
            return None
        if tr_x is None:
            score_method = 'column'
        ...
        if self.log_dir is not None:
            ...
            self._log_stats(f'Predicting Testing Time: {score_method} [s]', t)
            self._log_stats(f'Accuracy: {score_method}', score)

        return score

    def get_probabilities(self, store=True):
        """Get the probabilities associated with each feature.

        This technique uses the max across probabilities to form the global
        probabilities. This method should be called after fitting the SP.
    ...
    def transform(self, x):
        """Transform each value in x to be a probability.
    ...
    def reconstruct_input(self, x=None):
        ...
        Uses only the stored permanences and the set of active columns.

        The maximization of probabilities approach is used. This method must be
