...
def create_runner(command, runner_path, template=runner_template,
                  job_name='job', stdio_path='stdio.txt',
                  stderr_path='stderr.txt', partition_name='debug',
                  time_limit='00-06:00:00', memory_limit=512):
...
def execute_runner(runner_path, queue_limit=7000):
    ...
    """
    def queue_free():
        """Check the queue to see if there is room to add more jobs.
...
class ParamGenerator(object):
    """Build parameters properly bounded for the SP.

    Item generation follows scikit-learn's ParameterSampler; therefore, the
    order is guaranteed.
        ...
        for i in range(1, self.niter + 1):
            ...
            for key in missing:
                if hasattr(self.param_distributions[key], 'rvs'):
                    ...
                    param[key] = value[randint(0, len(v)).rvs()] #TODO
...
class ConfigGenerator(object):
    ...
    This class creates full dictionary configuration objects for the provided
    parameters.
    ...
    def get_config(self):
        ...
        if log_dir is not None:
            ...
            log_fmt = os.path.join(log_dir,
                                   f'{{0:0{len(str(nconfigs))}d}}-\
                                     {{0:0{len(str(self.ntrials))}d}}')
        ...
        for i, items in enumerate(product(*list(dynamic_params.values())), 1):
            ...
            # Make a useful log directory
            if log_dir is not None:
                config['log_dir'] = log_fmt.format(i, 1)
