...
def p_a1(npsyns, ninputs):
    """Probability of selecting one input given ninputs and npsyns attempts.
...
def p_a2(npsyns, ninputs):
    ...
    This uses a binomial distribution.
...
def main(ncols, npsyns, ninputs, density, seg_th, syn_th, ntrials=100,
    ...
    print(f'Probability of all inputs being selected: \
            {(1-p)*100:2.2f}')
    print(f'Expected inputs not seen: \
            {int(p * ninputs)}')
    print(f'Expected number of columns connected to an input: \
            {int(e_b(ncols, npsyns, ninputs))}')
    ...
    for _ in range(ntrials):
        ...
        y4 += (a >= seg_th).sum()
        y5 += (b >= seg_th).sum()
    print(f'Average number of missing inputs: \
            {(y0 / ntrials):.2f}')
    print(f'Average number of columns connected to an input: \
            {(y1 / ntrials):.2f}')
    print(f'Average number of active inputs per column: \
            {(y2 / ntrials):.2f}')
    print(f'Average number of active connected inputs per column: \
            {(y3 / ntrials):.2f}')
    print(f'Number of columns with active inputs >= seg_th \
            {(y4 / ntrials):.2f}')
    print(f'Number of columns with active connected inputs >= seg_th \
            {(y5 / ntrials):.2f}')
