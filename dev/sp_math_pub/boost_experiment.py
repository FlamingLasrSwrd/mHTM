...
def _phase3(self):
    """Compute normal phase 3, but with tracking the boost changes.

    Double commented lines are new.
    """
    # Update permanences
    self.p = np.clip(
        self.p +
            (self.c_pupdate * self.y[:, 0:1] * self.x[self.syn_map] -
                self.pdec * self.y[:, 0:1]),
        0,
        1)
    ...
    if self.disable_boost is False:
        ...
        with open(os.path.join(self.out_path, 'overlap_boost.csv'), 'a') as f:
            writer = csv.writer(f)
        ...
        p = os.path.join(self.out_path, 'permanence_boost.csv')
        with open(p, 'a') as f:
            writer = csv.writer(f)
    ...
    def get_data(p):
        ...
        for d in os.listdir(p):
            ...
            if os.path.isdir(npath):
                overlap.append(average(
                    os.path.join(npath, 'overlap_boost.csv')))
                permanence.append(average(
                    os.path.join(npath, 'permanence_boost.csv')))
