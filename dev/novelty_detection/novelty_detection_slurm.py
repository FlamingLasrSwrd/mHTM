...
def base_experiment(config, pct_noise=0.15, noverlap_bits=0, ntrials=10,
    ...
    outlier_base[np.random.choice(choices,
                                  x_ds.nactive - noverlap_bits,
                                  False)] = 1
    ...
    with open(os.path.join(base_dir, 'results.pkl'), 'wb') as f:
        pickle.dump((sp_x_results, sp_y_results, svm_x_results,
                     svm_y_results), f, pickle.HIGHEST_PROTOCOL)
...
if __name__ == '__main__':
    ...
    if local:
        log_dir = os.path.join(user_path,
                               'scratch',
                               'novelty_detection',
                               '0.15-0')
