...
def base_experiment(config, ntrials=1, seed=123456789):
    ...
    outliers = [np.random.permutation(tr_x[tr_y == i])[:ntest]
                for i in range(1, 10)]
    ...
    for nt in range(ntrials):
        ...
        for i, (a, b, c, d, e, f) in enumerate(zip(
                u_y_te, o_y_te, c_y_te, u_sp_y_te, o_sp_y_te, c_sp_y_te), 1):
        ...
        for i, j in enumerate(range(1, 10)):
            sp._log_stats(f'Base Train to Novelty {j} Uniqueness', u_sp[i])
            sp._log_stats(f'Base Train to Novelty {j} Overlap', o_sp[i])
            sp._log_stats(f'Base Train to Novelty {j} Correlation', c_sp[i])
        ...
        for i, j in enumerate(range(1, 10)):
            sp._log_stats(f'SP % Correct Novelty Class {j}', clf_y_te[i])
            sp._log_stats('SVM % Correct Novelty Class {j}', svm_y_te[i])
...
if __name__ == '__main__':
    ...
    if local:
        log_dir = os.path.join(user_path, 'scratch',
                               'novelty_experiments', 'mnist')
