...
class InvalidCategoryEncoderNumberCategories(BaseException):
    ...
    def __init__(self, ncategories):
        ...
        """
        self.msg = wrap_error(f'The requested number of categories, \
            {ncategories}, is too low. There must be at least one category.')
...
class InvalidCategoryEncoderCategory(BaseException):
    ...
    def __init__(self, desired, valid):
        ...
        """
        self.msg = wrap_error(f'The category, {desired}, is not valid. \
            Valid categories are: {", ".join(str(c) for c in valid)}')
...
class Category(Encoder):
    ...
    This encoder uses a multi-hot encoding for each category.
    ...
    def __init__(self, num_categories, active_bits=None, num_bits=None,
        ...
        self.encoder = Scalar(num_bins=self.num_categories,
                              bin_overlap=0,
                              min_val=0,
                              max_val=self.num_categories - 1,
