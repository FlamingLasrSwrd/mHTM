...
class Multi(Encoder):
    """This encoder combines multiple encoders together.

    The final encoding is a concatenation of outputted streams.
    """
    ...
    def __init__(self, *encoders, **kargs):
        ...
        super(Multi, self).__init__(
            sum(e.num_bits for e in self.encoders), **kargs)
    ...
    def _encode(self, values):
        """Encode each value with the specified encoder.
    ...
    def encode(self, data=None):
        """Use a generator to yield each encoded bit.

        This supports being able to encode a list of values, where each value
        will be sequentially encoded. This function can also encode single
        values.
    ...
    def bind_data(self, raw_data):
        """Bind the raw data to the encoder.
    ...
    def free_data(self):
        """Delete the data stored in the encoder."""
        for encoder in self.encoders:
            encoder.free_data()
