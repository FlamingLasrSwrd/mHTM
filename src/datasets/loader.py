...
class MNISTCV(object):
    ...
    A stratified shuffle split is used for the division of the data.
    """
    ...
    def __init__(self, tr_y, te_y, train_size=800, test_size=200, nsplits=5,
        ...
        @param train_size: The size of the train set, as a percentage or
        integer.
    ...
    def __iter__(self):
        ...
        Each call yields a tuple containing the current training and testing
        split.
        """
    ...
    def __len__(self):
        """Allow the generator's length to be obtained.

        Only valid upon the first call.
    ...
    def _create_generator(self):
        ...
        Yield a tuple containing the current training and testing split.
        """
...
def load_mnist(threshold=255/2):
    ...
    """
    # Get the raw data
    pkgdir = os.path.dirname(pkgutil.get_loader('mHTM.datasets').path)
    p = os.path.join(pkgdir, 'mnist.pkl')
    with open(p, 'rb') as f:
        (tr_x, tr_y), (te_x, te_y) = pickle.load(f, encoding='latin1')
...
class SPDataset(object):
    ...
    The data is available once the object is initialized. Simply access it by
