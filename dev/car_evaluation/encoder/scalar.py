...
class InvalidScalarEncoderLimits(BaseException):
    """Exception if minimum value is >= to the maximum value."""
    ...
    def __init__(self, min_value, max_value):
        ...
        """
        self.msg = wrap_error(f'The minimum value, {min_value}, \
            is greater than or equal to the maximum value, {max_value}.')
...
class InvalidScalarEncoderBinOverlap(BaseException):
    ...
    def __init__(self, bin_overlap):
        ...
        encoder.
        """
        self.msg = wrap_error(f'The specified bin overlap, {bin_overlap}, \
...
class InvalidScalarEncoderBins(BaseException):
    ...
    def __init__(self, num_bins):
        ...
        """
        self.msg = wrap_error(
            f'The specified number of bins, {num_bins}, is less than 1.')
...
class InvalidScalarEncoderBits(BaseException):
    ...
    def __init__(self, active_bits):
        ...
        """
        self.msg = wrap_error(
            f'The specified number of active bits, {active_bits}, \
                is less than 1.')
...
class InvalidScalarEncoderNumberBits(BaseException):
    ...
    def __init__(self, nbits):
        ...
        """
        self.msg = wrap_error(f'The requested number of bits, {nbits}, is too'
            ' low. Increase the number of bits and try again.')
...
class InvalidScalarEncoderDecodeOption(BaseException):
    ...
    def __init__(self, decode, valid_decodes):
        ...
        """
        self.msg = wrap_error(
            f'The requested decoding type, {decode}, is invalid.'
            ' Please choose a decoding type from the following: '
            '{', '.join(sorted(valid_decodes))}')
...
class Scalar(Encoder):
    """Quantizes scalars and performs a multi-hot encoding of each scalar."""

    def __init__(self, num_bins, min_val, max_val, active_bits=None,
                 num_bits=None, bin_overlap=0., wrap=False, decode_type="all",
                 **kargs):
        ...
        def simple_initialize():
            """Initialize the scalar encoder using the simplified settings.
        ...
        def initialize_params(num_bits=num_bits, active_bits=active_bits):
            ...
            This function handle all possible initialization configurations and
            ensures that the encoder will be created properly.
    ...
    def _get_bin(self, value):
        """Determine which bin the value lies in.
    ...
    def _decode(self, encoding):
        ...
        It is possible that there is no clear choice as to what the decoded
        value should be. Determining the correct winner goes as follows:
    ...
    def encoding_to_str(self, encoding):
        ...
        return ''.join(f'{fmt(i)}1' if r == 1 else
                       f'{fmt(i)}.' for i, r in enumerate(encoding))
    ...
    def get_all_inputs(self):
        ...
        Note that there can be more inputs that will be created here, but these
        inputs will guarantee that each bin will be represented. To effectively
        use this method it should be noted that each input maps directly to a
        bin integer.
