...
def dump_results(bp):
    ...
    with open(os.path.join(bp, 'full_results.pkl'), 'wb') as f:
        pickle.dump((np.array(sp_x_results), np.array(sp_y_results),
                     np.array(svm_x_results), np.array(svm_y_results),
                     np.array(param)), f, pickle.HIGHEST_PROTOCOL)
...
def make_3d_plot(p):
    ...
    ix = np.array(pd.DataFrame(param).sort_values(
            [0, 1], ascending=[True, True]).index).astype('i')
    param = param[ix]
    sp_x = sp_x[ix]
    sp_y = sp_y[ix]
    svm_x = svm_x[ix]
...
def make_2d_plot(p, noise=None, overlap=None):
    ...
    dir = os.path.dirname(p)
    plot_error((x, x),
               (np.median(sp_x[ix], 1), np.median(svm_x[ix], 1)),
               ('SP', 'SVM'),
               (compute_err(sp_x[ix]), compute_err(svm_x[ix])),
               f'% {name.capitalize()}',
               '% Error',
               xlim=(-5, 105),
               ylim=(-5, 105),
               show=False,
               out_path=os.path.join(dir, f'train-{name}.png'))
    plot_error((x, x),
               (np.median(sp_y[ix], 1), np.median(svm_y[ix], 1)),
               ('SP', 'SVM'),
               (compute_err(sp_y[ix]), compute_err(svm_y[ix])),
               f'% {name.capitalize()}',
               '% Error',
               xlim=(-5, 105),
               ylim=(-5, 105),
               show=False,
               out_path=os.path.join(dir, f'test-{name}.png'))
