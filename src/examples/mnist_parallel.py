...
def main(log_dir, ntrain=800, ntest=200, niter=10, nsplits=5,
    ...
    """Build the info needed to perform CV on a subset of the MNIST dataset.
...
def main_local(log_dir, ntrain=800, ntest=200, niter=5, nsplits=3,
    ...
    """Perform CV on a subset of the MNIST dataset.

    Performs parallelizations on a local machine.
    ...
    x, y, kargs, params, cv = main(log_dir, ntrain, ntest, niter, nsplits, seed)
...
def main_slurm(log_dir, ntrain=800, ntest=200, niter=5, nsplits=3,
    ...
    """Perform CV on a subset of the MNIST dataset, using SLRUM.

    Iterations will be run in complete parallel. Splits within an iteration
    ...
    for i in range(1, niter + 1):
        ...
        # Create the runner
        pkgdir = os.path.dirname(pkgutil.get_loader('mHTM.examples').path)
        mnist_runner_path = os.path.join(pkgdir, 'mnist_runner.py')
