...
def create_base_config(base_dir, experiment_name, global_inhibition=True,
    ...
    return {
        ...
        'log_dir': os.path.join(
            base_dir,
            'global' if global_inhibition else 'local',
            experiment_name)}


def run_experiment(experiments, base_dir,
                   nsamples=500, nbits=100, pct_active=0.4, pct_noise=0.15,
                   seed=123456789, ntrials=10, partition_name='debug',
                   this_dir=os.getcwd()):
        ...
        for i, global_inhibition in enumerate((True, False)):
            ...
            for config in config_gen.get_config():
                ...
                with open(os.path.join(dir, 'dataset.pkl'), 'wb') as f:
                    ...
                    pickle.dump(
                        (uniqueness_data, overlap_data, correlation_data),
                        f,
                        pickle.HIGHEST_PROTOCOL)

                # Create the runner
                this_path = os.path.join(this_dir, 'parameter_exploration.py')
                command = 'python "{0}" "{1}" {2} {3}'.format(this_path,
                                                              dir,
                                                              ntrials,
                                                              seed)
...
def first_order_effects(base_dir, nsamples=500, nbits=100, pct_active=0.4,
                        pct_noise=0.15, seed=123456789, ntrials=10,
                        partition_name='debug', this_dir=os.getcwd()):
    """Explore first order params effects on the quality of the SP's SDRs.

    This function uses SLURM to conduct the experiments.
    ...
    run_experiment(experiments, base_dir, nsamples, nbits, pct_active,
                   pct_noise, seed, ntrials=ntrials,
                   partition_name=partition_name, this_dir=this_dir)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        first_order_effects(os.path.join(base_path, 'first_order'),
                            ntrials=ntrials, this_dir=this_dir,
                            partition_name=partition_name)
    elif len(sys.argv) == 4:
