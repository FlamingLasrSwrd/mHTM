...
def get_results(bp, score_method='column'):
    ...
    for p in sorted(os.listdir(bp)):
        ...
        with open(os.path.join(bp, p, 'stats.csv'), 'r') as f:
            reader = csv.reader(f)
            for row in reader:
                key, value = row
                if method in key:
...
def get_top_paths(bp, k=10):
    """Get the top paths for each method.

    This method uses the mean across the CV runs.
...
def get_top_path(bp):
    """Get the top path out of all methods.
...
def launch_top_runs(top_paths, bp, command,
                    auto_pupdate=False, partition_name='debug',
                    time_limit='04-00:00:00', memory_limit=2048):
    """Launch the top runs.
...
if __name__ == '__main__':
    ...
    command = f'python {mnist_runner_path}'
