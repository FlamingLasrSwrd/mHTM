...
def compute_axis_bounds(x, multiplier=1):
    """Compute the bounds for the axis.

    It is assumed that the data starts at 0, unless the data goes below zero.
    The bounds are based off the standard deviation.
...
def plot_epoch(y_series, series_names=None, y_errs=None, y_label=None,
               title=None, semilog=False, out_path=None, show=True,
               start_epoch=1):
    """Plot various types of data against training epochs.

    Each item in the series should correspond to a single data point for that
    epoch.
...
def plot_error(x_series, y_series, series_names=None, y_errs=None,
               x_label=None, y_label=None, title=None, semilog=False,
               out_path=None, show=True, xlim=None, ylim=None, ax=None,
               legend=True):
...
def plot_line(x_series, y_series, series_names=None, x_label=None,
              y_label=None, title=None, semilog=False, out_path=None,
...
def plot_image(x, shape, title=None, out_path=None, show=True):
    ...
    It is assumed that the image has been normalized (0, 1).
...
def plot_grid_images(X, shape, nrows, ncols, title=None, labels=None,
    ...
    """Construct a grid of grayscale images.

    It is assumed that the images have been normalized (0, 1).
...
def plot_compare_images(x_series, shape, title=None, series_labels=None,
    ...
    It is assumed that the images have been normalized (0, 1). The images will
    be plotted row-wise based off the order they were provided.
    ...
    if series_labels is None:
        series_labels = [[None] * nsamples for _ in range(nseries)]
...
def plot_surface(x, y, z, x_label=None, y_label=None, z_label=None,
                 title=None, out_path=None, show=True, azim=-124, elev=38,
                 vmin=0, vmax=100):
    """Plot surfaces.
    ...
    if title is not None:
        plt.title(title)
    if x_label is not None:
        ax.set_xlabel(x_label, labelpad=50)
    if y_label is not None:
        ax.set_ylabel(y_label, labelpad=60)
...
def plot_surface_video(x, y, z, out_dir, x_label=None, y_label=None,
    ...
    """Plot a bunch of views for a surface.
