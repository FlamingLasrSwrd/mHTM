...
class SPMetrics(object):
    """This class allows for an unbiased method for studying the SP.

    The items in this class are specifically designed for characterizing the
    quality of SDRs. Custom scoring metrics are included for determining how
    good the SP's output SDRs are.
    ...
    def compute_uniqueness(data):
        """Compute the percentage of unique SDRs in the given dataset.

        This method will return the percentage of unique SDRs.
        It is normalized such that if all SDRs are unique it will return one
        and if no SDRs are alike it will return zero.
        A score of zero indicates that exactly the same SDRs were produced.
    ...
    def compute_total_similarity(data, confidence_interval=0.9):
        """Compute the degree of similarity between SDRs.

        This method computes the average activation of each bit across the SDRs.
        For a bit to be similar across all SDRs it must be active at least
        confidence_interval% of the time or it must be inactive at least
        (1 - confidence_interval)% of the time. If each bit in the SDR meets
        that criteria, the SDRs are said to be 100% similar (this method
        returns a 1).
    ...
    def compute_one_similarity(data, confidence_interval=0.9):
        """Compute the degree of '1' similarity between SDRs.

        This method computes the average activation of each bit across the SDRs.
        For a bit to be similar across all SDRs it must be active at least
        confidence_interval% of the time. This method only looks at the
        similarity of the active bits. If those bits in the SDRs meet the above
        criteria, the SDRs are said to be 100% similar (this method returns a 1).
    ...
    def compute_zero_similarity(data, confidence_interval=0.9):
        """Compute the degree of '0' similarity between SDRs.

        This method computes the average activation of each bit across the SDRs.
        For a bit to be similar across all SDRs it must be inactive at least
        ...
        above criteria, the SDRs are said to be 100% similar (this method
        returns a 1).
    ...
    def compute_dissimilarity(data, confidence_interval=0.9):
        """Compute the degree of dissimilarity between SDRs.

        This method is used to report the percentage of bits that do not fall
        into an active or inactive category (see the compute_one_similarity and
    ...
    def compute_overlap(data):
        """Compute the average normalized overlap across all vector pairs.

        The overlap is normalized relative to the largest overlap possible for
        the given data.
    ...
    def compute_distance(data, metric='correlation'):
        """Compute the average distance between all vector pairs.

        Refer to
        U{pdist<http://docs.scipy.org/doc/scipy-0.16.0/reference/generated/scipy.spatial.distance.pdist.html>}
