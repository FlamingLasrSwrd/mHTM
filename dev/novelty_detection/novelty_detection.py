...
def base_experiment(pct_noise=0.15, noverlap_bits=0, exp_name='1-1',
    ...
    outlier_base[np.random.choice(
        choices, x_ds.nactive - noverlap_bits, False)] = 1
    ...
    for i in range(ntrials):
        ...
        if verbose:
            ...
            print((fmt_s.format('Overlap', o_x_tr, o_x_te, o_y_te,
                                o_sp_x_tr, o_sp_x_te, o_sp_y_te)))
            print((fmt_s.format('Correlation', c_x_tr, c_x_te,
                                c_y_te, c_sp_x_tr, c_sp_x_te, c_sp_y_te)))
        ...
        sp._log_stats('Base Train to Base Test Uniqueness',
                      u_sp_base_to_x_te)
        sp._log_stats('Base Train to Base Test Overlap',
                      o_sp_base_to_x_te)
        sp._log_stats('Base Train to Base Test Correlation',
                      c_sp_base_to_x_te)
        sp._log_stats('Base Train to Novelty Test Uniqueness',
                      u_sp_base_to_y_te)
        sp._log_stats('Base Train to Novelty Test Overlap',
                      o_sp_base_to_y_te)
...
def main(ntrials=10, seed=123456798):
    ...
    results = Parallel(n_jobs=-1)(delayed(base_experiment)(
        noise, 0, f'noise-{i}', ntrials, False, seed)
    ...
    svm_noise_x_err1, svm_noise_x_err2 = [], []  # TODO: empty list generator
    ...
    # Vary the overlaps
    results = Parallel(n_jobs=-1)(delayed(base_experiment)(
        0.35, overlap, 'overlap-{i}', ntrials, False, seed)
    ...
    with open(os.path.join(p, 'results.pkl'), 'wb') as f:
        ...
        pickle.dump((svm_noise_x_err, svm_noise_y_err),
                    f, pickle.HIGHEST_PROTOCOL)
        pickle.dump((overlap_x, overlap_y), f, pickle.HIGHEST_PROTOCOL)
        pickle.dump((svm_overlap_x, svm_overlap_y),
                    f, pickle.HIGHEST_PROTOCOL)
        pickle.dump((overlap_x_err, overlap_y_err),
                    f, pickle.HIGHEST_PROTOCOL)
        pickle.dump((svm_overlap_x_err, svm_overlap_y_err),
                    f, pickle.HIGHEST_PROTOCOL)

    # Make the plots
    plot_error((pct_noises * 100, pct_noises * 100),
               (noise_x, svm_noise_x),
               ('SP', 'SVM'),
               (noise_x_err, svm_noise_x_err),
               '% Noise',
               '% Error',
               'Noise: Base Class',
    ...
    plot_error((noverlap_pct, noverlap_pct),
               (overlap_x, svm_overlap_x),
               ('SP', 'SVM'),
               (overlap_x_err, svm_overlap_x_err),
               ...
               show=False)
    plot_error((noverlap_pct, noverlap_pct),
               (overlap_y, svm_overlap_y),
               ('SP', 'SVM'),
               (overlap_y_err, svm_overlap_y_err),
               '% Overlapping Bits',
               '% Error',
