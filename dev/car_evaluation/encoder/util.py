...
def get_shape(seq):
    """Return a list containing the dimensions of an N-Dimensional sequence.
    ...
    """
# TODO: python never copies data; seq will be mutated by this function
    # Initializations
    l = seq
    shape = []
...
def flatten(seq):
    """Flattens a sequence of elements.
...
def mkudir(base_dir, base_name=None):
    ...
    try:
        if base_name is not None:
            sub_dir = os.path.join(base_dir, f'{base_name}-{timestamp()}')
        else:
            sub_dir = os.path.join(base_dir, f'{timestamp()}')
        os.makedirs(sub_dir)
        return sub_dir
    except:
