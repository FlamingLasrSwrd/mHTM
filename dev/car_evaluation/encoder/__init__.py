...
class UnsupportedEncoder(BaseException):
    ...
    def __init__(self, name):
        ...
        """
        self.msg = wrap_error(
            f'The desired encoder, {name}, is not supported. \
              Please request a valid encoder.')
...
def get_encoder(type, **kargs):
    """Create an encoder of the appropriate type and returns an instance of it.
...
def is_finite(encoder):
    """Determine if the encoder has a finite number of bins.

    Technically all encoders do, but this refers to those that have a feasibly
    finite number of bins. For example, a scalar encoder with 100 bins has a
    small number of bins (100), thus it is finite. A unity encoder with only 10
    bits has a large number of bins (10!), thus it is not finite.
