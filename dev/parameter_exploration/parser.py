...
def get_results(base_path, config_keys, x=None, y=None):
    ...
    for path in sorted(os.listdir(base_path)):
        ...
        # Get the JSON config
        with open(os.path.join(base_path, path, 'config.json'), 'r') as f:
            config = json.load(f)
        ...
        # Read in the results
        data = []
        with open(os.path.join(base_path, path, 'stats.csv'), 'r') as f:
            reader = csv.reader(f)
            for row in reader:
...
def main(root_dir):
    ...
    for dir in get_sorted_dirs(
            [os.path.join(root_dir, p) for p in os.listdir(root_dir)]):
        parameter_names, experiment_name = experiment_map[os.path.basename(dir)]
        ...
        if experiment_name == prev_name:
            ...
            if not ((x is None) and (y is None)):
                p = os.path.join(root_dir, f'{prev_name}.pkl')
                with open(p, 'wb') as f:
